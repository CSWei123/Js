...
window.player = this.player
    ...
    this.Key = this.physics.add.sprite(Key.x, Key.y, 'keyPart1').play("Glow")
...
class Player {
  ...
  teleport(newX, newY) {
      this.x = newX;
      this.y = newY;
      console.log(`Player teleported to (${this.x}, ${this.y})`);
  }
}
...
function gameLoop(player, items, teleportLocation) {
  for (let item of items) {
      if (checkCollision(player, item)) {
          if (item.type === 'teleporter') {
              player.teleport(teleportLocation.x, teleportLocation.y);
          }
...
let items = [
  new Item(5, 5, 'teleporter'),
  new Item(10, 10, 'coin')
];
let teleportLocation = { x: 20, y: 20 };

// Simulate player movement
player.x = 5;
player.y = 5;

gameLoop(player, items, teleportLocation);
